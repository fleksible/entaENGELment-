...
class ColorGenerator:
    ...
    def create_dynamic_colormap(self, base_hsv: Tuple[float, float, float], 
                               shift_factor: float, name: str = "dynamic") -> LinearSegmentedColormap:
    ...
    def create_sensor_based_colormap(self, sensor_data: Dict[str, float], 
                                    fractal_zoom: float, name: str = "sensor_based") -> LinearSegmentedColormap:
        ...
        s_base = max(0.4, min(0.9, 0.6 + accel_magnitude * 0.05))  # Sättigung basierend auf Beschleunigung
        v_base = max(0.6, min(0.95, 0.8 + np.log10(max(1.0, fractal_zoom)) * 0.05))  # Hellwert basierend auf Zoom
    ...
    def create_resonant_gradient(self, base_frequency: float, harmonic_ratio: float, 
                                n_colors: int = 256) -> List[Tuple[float, float, float]]:
    ...
    def create_golden_ratio_colormap(self, base_hue: float = 0.0, 
                                    name: str = "golden_ratio") -> LinearSegmentedColormap:
    ...
    def create_quantum_colormap(self, energy_levels: int = 7, 
                               name: str = "quantum") -> LinearSegmentedColormap:
