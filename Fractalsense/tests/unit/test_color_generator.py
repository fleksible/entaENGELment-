...
class TestDynamicColormap:
    ...
    def test_creates_from_hsv(self, color_generator):
        ...
        cmap = color_generator.create_dynamic_colormap(
            base_hsv=(0.5, 0.8, 0.9), shift_factor=0.5, name="test_dynamic"
        )
        assert cmap is not None
    ...
    def test_returns_colormap(self, color_generator):
        ...
        cmap = color_generator.create_dynamic_colormap(
            base_hsv=(0.3, 0.7, 0.8), shift_factor=0.3, name="test_dynamic_2"
        )
        color = cmap(0.5)
        assert len(color) == 4  # RGBA
    ...
    def test_shift_factor_affects_output(self, color_generator):
        ...
        cmap1 = color_generator.create_dynamic_colormap(
            base_hsv=(0.5, 0.8, 0.9), shift_factor=0.1, name="test_shift_1"
        )
        cmap2 = color_generator.create_dynamic_colormap(
            base_hsv=(0.5, 0.8, 0.9), shift_factor=0.9, name="test_shift_2"
        )
...
class TestSensorBasedColormap:
    ...
    def test_creates_from_sensor_data(self, color_generator, sample_sensor_data):
        ...
        cmap = color_generator.create_sensor_based_colormap(
            sensor_data=sample_sensor_data, fractal_zoom=2.0
        )
        assert cmap is not None
...
class TestGoldenRatioColormap:
    ...
    def test_uses_phi_in_calculation(self, color_generator):
        ...
        cmap = color_generator.create_golden_ratio_colormap(base_hue=0.0, name="golden_test")
    ...
    def test_different_base_hues(self, color_generator):
        ...
        cmap1 = color_generator.create_golden_ratio_colormap(base_hue=0.0, name="golden_0")
        cmap2 = color_generator.create_golden_ratio_colormap(base_hue=0.5, name="golden_05")
...
class TestResonantGradient:
    ...
    def test_creates_gradient_list(self, color_generator):
        ...
        colors = color_generator.create_resonant_gradient(base_frequency=1.0, harmonic_ratio=1.5)
    ...
    def test_custom_color_count(self, color_generator):
        ...
        colors = color_generator.create_resonant_gradient(
            base_frequency=1.0, harmonic_ratio=1.5, n_colors=128
        )
        assert len(colors) == 128

    def test_rgb_values_in_range(self, color_generator):
        """All RGB values should be in [0, 1] range."""
        colors = color_generator.create_resonant_gradient(base_frequency=2.0, harmonic_ratio=1.618)
        for r, g, b in colors:
            assert 0 <= r <= 1
            assert 0 <= g <= 1
            assert 0 <= b <= 1
    ...
    def test_harmonic_ratio_affects_output(self, color_generator):
        ...
        colors1 = color_generator.create_resonant_gradient(base_frequency=1.0, harmonic_ratio=1.0)
        colors2 = color_generator.create_resonant_gradient(base_frequency=1.0, harmonic_ratio=2.0)

        # At least some colors should differ
        differences = sum(
            1 for c1, c2 in zip(colors1, colors2) if not np.allclose(c1, c2, atol=0.01)
        )
...
class TestRGBBounds:
    ...
    def test_dynamic_colormap_rgb_in_range(self, color_generator):
        ...
        for h in [0.0, 0.3, 0.6, 0.9]:
            cmap = color_generator.create_dynamic_colormap(
                base_hsv=(h, 0.8, 0.9), shift_factor=0.5, name=f"dynamic_test_{h}"
            )
...
class TestEdgeCases:
    ...
    def test_extreme_shift_factors(self, color_generator):
        ...
        for shift in [0.0, 0.001, 100.0]:
            cmap = color_generator.create_dynamic_colormap(
                base_hsv=(0.5, 0.8, 0.9), shift_factor=shift, name=f"extreme_shift_{shift}"
            )
            assert cmap is not None
