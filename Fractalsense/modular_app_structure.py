...
class ModuleRegistry:
    ...
    def register_module_class(self, module_class: Type[ModuleInterface], module_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Modul '{module_name}' (v{temp_instance.version}) erfolgreich registriert."
            )
            return True
            ...
            if os.path.isdir(module_path) and os.path.exists(
                ...
                try:
                    ...
                    for name, obj in inspect.getmembers(module):
                        if (
                            inspect.isclass(obj)
                            and issubclass(obj, ModuleInterface)
                            and obj != ModuleInterface
                        ):
        ...
        for module_name in self._module_load_order:
            if module_name not in self._modules:
                try:
                    ...
                    logger.error(
                        f"Fehler bei der Initialisierung des Moduls '{module_name}': {str(e)}"
                    )
                    return False
            ...
            if module_name not in visited:
                ...
                for dependency in self._module_dependencies.get(module_name, []):
                    if dependency in self._module_classes:
                        ...
                        logger.warning(
                            f"Abhängigkeit '{dependency}' für Modul '{module_name}' nicht gefunden."
                        )
    ...
    def load_config(self) -> bool:
        ...
        try:
            if os.path.exists(self._config_file):
                ...
                logger.warning(
                    f"Konfigurationsdatei '{self._config_file}' nicht gefunden. Verwende Standardkonfiguration."
                )
                return False
        except Exception as e:
