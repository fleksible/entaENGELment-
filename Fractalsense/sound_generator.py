...
class SoundGenerator:
    ...
    def generate_fm_wave(
        ...
        wave = amplitude * np.sin(
            2 * np.pi * carrier_freq * t + modulation_index * np.sin(2 * np.pi * modulator_freq * t)
        )
        return wave

    def generate_resonant_wave(
        self,
        base_frequency: float,
        ...
        frequencies = [
            ...
            base_frequency
            * phi**2
            % (base_frequency * 2),  # Moduliert, um im hörbaren Bereich zu bleiben
            base_frequency / phi,
        ]
    ...
    def apply_envelope(
        self, wave: np.ndarray, attack: float, decay: float, sustain: float, release: float
    ) -> np.ndarray:
        ...
        if sustain_samples > 0:
            envelope[
                attack_samples + decay_samples : attack_samples + decay_samples + sustain_samples
            ] = sustain
    ...
    def apply_filter(
        self, wave: np.ndarray, filter_type: str, cutoff_freq: float, resonance: float = 1.0
    ) -> np.ndarray:
    ...
    def generate_chord(
        self, base_frequency: float, chord_type: str, duration: float, amplitude: float = 1.0
    ) -> np.ndarray:
